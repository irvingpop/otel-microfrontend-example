...
export const TransitWidget: React.FC = () => {
  const telemetry = new MicrofrontendTelemetry('transit-service')
  const [data, setData] = useState<TransitData | null>(null)
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    const timer = setTimeout(() => {
      // Load transit data with telemetry
      telemetry.withSpan('transit.load_data', (span) => {
        setData(mockTransitData)
        setLoading(false)

        span.setAttributes({
          'transit.routes_count': mockTransitData.routes.length,
          'transit.alerts_count': mockTransitData.alerts.length,
          'transit.on_time_routes': mockTransitData.routes.filter(r => r.status === 'on-time').length,
          'transit.delayed_routes': mockTransitData.routes.filter(r => r.status === 'delayed').length,
          'transit.cancelled_routes': mockTransitData.routes.filter(r => r.status === 'cancelled').length
        })
      })

      // Notify parent that widget has loaded
      telemetry.notifyWidgetLoaded('transit', {
