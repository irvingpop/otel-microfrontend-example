...
import { getOrCreateSessionTrace, trackWidgetActivation, trackWidgetDeactivation, endSessionTrace, telemetryReady } from '../lib/telemetry'
...
export const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Wait for telemetry to be ready before creating session
    telemetryReady.then(() => {
      if (mounted) {
    ...
    // Clean up session trace on unmount
    return () => {
      mounted = false
      endSessionTrace()
    }
