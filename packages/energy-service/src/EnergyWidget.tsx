...
export const EnergyWidget: React.FC = () => {
  const telemetry = new MicrofrontendTelemetry('energy-service')
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      // Load energy data with telemetry
      telemetry.withSpan('energy.load_data', (span) => {
        span.setAttributes({
          'energy.current_usage': data.current,
          'energy.renewable_percentage': data.renewable,
          'energy.peak_today': data.peak,
          'energy.efficiency': data.efficiency,
          'energy.carbon_saved': data.carbonSaved
        })
      })

      // Notify parent that widget has loaded
      telemetry.notifyWidgetLoaded('energy', {
